...
//一个官方的Windows API（获取Windows系统底层运行时的各种状态信息）
...
// 全局函数指针用于让其他地方可以注入键鼠数据
extern fnKeyboardClassServiceCallback hkBridgeKeyboardClassServiceCallback;
extern fnMouseClassServiceCallback    hkBridgeMouseClassServiceCallback;
...
//键盘数据环形队列
typedef struct _KEYBOARD_INPUT_QUEUE {
	KEYBOARD_INPUT_DATA Data[1024];
	ULONG Count;//可读取数据个数
	ULONG WriteIndex;//默认为0，写入一个后，写入索引+1
	ULONG ReadIndex;//默认为0，读取一个后，读取索引+1
	KSPIN_LOCK Lock;
} KEYBOARD_INPUT_QUEUE, *PKEYBOARD_INPUT_QUEUE;
...
//鼠标鼠标环形队列
typedef struct _MOUSE_INPUT_QUEUE {
	MOUSE_INPUT_DATA Data[1024];
	ULONG Count;//可读取数据个数
	ULONG WriteIndex;//默认为0，写入一个后，写入索引+1
	ULONG ReadIndex;//默认为0，读取一个后，读取索引+1
	KSPIN_LOCK Lock;
} MOUSE_INPUT_QUEUE, *PMOUSE_INPUT_QUEUE;


//全局键鼠数据队列
extern KEYBOARD_INPUT_QUEUE gKeyboardDataQueue;
extern MOUSE_INPUT_QUEUE gMouseDataQueue;

//全局键鼠设备对象，hook到第一次保存使用
extern PDEVICE_OBJECT gKeyboardDeviceObject;
extern PDEVICE_OBJECT gMouseDeviceObject;
